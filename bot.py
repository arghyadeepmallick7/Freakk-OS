...
VCNotify, Moderation, Logging, CustomCommands, Announcements, Giveaways).
...
def render(
    ...
    *,
    ...
    if convert_literal_newlines:
        # Only the two-character escape. Real \n stays real \n.
        text = text.replace("\\n", "\n").replace("\\r\\n", "\r\n")
    if ctx:
...
class General(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction):
        lines = [
            ...
            "**Announcements** — `/announce create|send|schedule|cancel`",
...
class VCNotify(commands.Cog):
    ...
    async def add(self, interaction: discord.Interaction, voice: discord.VoiceChannel):
        # VC notifications always go to Discord's built-in text chat for that
        # exact voice channel. Never accept/store a separate text destination.
        await self.bot.db.execute(
            "INSERT OR REPLACE INTO vc_notify (guild_id, voice_channel_id, text_channel_id, join_message, leave_message) "
            "VALUES (?, ?, NULL, COALESCE((SELECT join_message FROM vc_notify WHERE guild_id=? AND voice_channel_id=?), ?), "
    ...
    async def test(self, interaction: discord.Interaction, voice: discord.VoiceChannel):
        ...
        ctx = {
            ...
            "channel_name": voice.name,
            "channel": voice.mention,
            "server": interaction.guild.name,
            "server_name": interaction.guild.name,
        }
        await voice.send(render(row["join_message"] or "", ctx))
        await interaction.response.send_message(
            embed=embed_success(f"Test sent to {voice.name}'s voice chat."), ephemeral=True)
    ...
    async def _notify(self, guild, vc, member, key):
        ...
        if not template:
            return
        # Always use the built-in text chat attached to this exact VC.
        # A separately configured text channel must never receive VCNotify messages.
        target = vc
        ctx = {
...
class Freakos(discord.Bot):
    ...
    async def setup_hook(self):
        ...
        for cog in (General, Welcome, Autorole, Autonick, Departure, ActionDM,
                    VCNotify, Moderation, GuildLogging, CustomCommands,
