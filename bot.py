...
class TicketCreateButton(discord.ui.View):
    ...
    @discord.ui.button(label='Open Ticket', style=discord.ButtonStyle.primary, custom_id='freakos:ticket:create', emoji='🎫')
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):
        guild = interaction.guild
        if not guild:
            return
        rows = await interaction.client.db.fetchall('SELECT name, emoji FROM ticket_types WHERE guild_id=? ORDER BY id', (guild.id,))
        if not rows:
            await interaction.response.send_message('No ticket types configured. Ask an admin to run `/ticket type`.', ephemeral=True)
            return
        opts = []
        for r in rows[:25]:
            opts.append(discord.SelectOption(label=r['name'][:100], emoji=r['emoji'] or None, value=r['name'][:100]))
        select = discord.ui.Select(placeholder='Pick a ticket type…', options=opts)
        async def cb(sel_interaction: discord.Interaction): await _open_ticket(sel_interaction, sel_interaction.data['values'][0])
        select.callback = cb
...
async def _open_ticket(interaction: discord.Interaction, ttype: str):
    ...
    overwrites = {guild.default_role: discord.PermissionOverwrite(view_channel=False), interaction.user: discord.PermissionOverwrite(view_channel=True, send_messages=True, attach_files=True, read_message_history=True), guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, manage_channels=True, manage_permissions=True)}
    if support_role:
...
def register_all_commands(bot: Freakos):
    ...
    async def t_setup(interaction: discord.Interaction):
        if not await require_admin(interaction):
            return
        await interaction.response.send_message('Use `/ticket type` to add a ticket type, then `/ticket panel` to place the panel.', ephemeral=True)
    ...
    async def t_panel(interaction: discord.Interaction, channel: Optional[discord.TextChannel]=None, title: Optional[str]=None, description: Optional[str]=None):
        ...
        embed = make_embed(title=title or '🎫 Support Tickets', description=description or 'Click the button below to open a ticket.')
    ...
    @app_commands.describe(name='Name', category='Category', support_role='Support role', emoji='Optional emoji', message='Intro message (optional)')
    ...
    async def t_transcript(interaction: discord.Interaction):
        ...
        await interaction.response.send_message('📄 Transcript:', file=discord.File(buf, filename=f'transcript-{interaction.channel.id}.txt'))
