...
async def _post_ticket_close_log(bot: "Freakos", guild: discord.Guild, channel, row, closed_by, reason: str):
    """Posts the 'Ticket Closed' summary embed + auto transcript to the configured ticket-logs channel."""
    ...
    if not log_ch:
        return
    claimed = f"<@{row['claimed_by']}>" if row["claimed_by"] else "Not claimed"
    e = discord.Embed(title="Ticket Closed", color=0x57F287, timestamp=now_utc())
    e.add_field(name="🆔 Ticket ID", value=str(row["id"]))
    e.add_field(name="✅ Opened By", value=f"<@{row['user_id']}>")
    e.add_field(name="🔒 Closed By", value=closed_by.mention)
    e.add_field(name="🕐 Open Time", value=fmt_dt_human(parse_iso(row["created_at"])))
    e.add_field(name="👤 Claimed By", value=claimed)
    e.add_field(name="❓ Reason", value=reason or "No reason specified", inline=False)
    try:
...
def register_all_commands(bot: Freakos):
    ...
    @ticket.command(name="logs", description="Set the channel where closed-ticket logs & transcripts are posted.")
    ...
    async def t_close(interaction: discord.Interaction, reason: str = "No reason specified"):
        ...
        try:
            ...
            pass
        await _post_ticket_close_log(bot, interaction.guild, interaction.channel, row, interaction.user, reason)
        delay = await _ticket_delete_delay(db, interaction.guild.id)
        if delay > 0:
            await interaction.response.send_message(
    ...
    async def t_transcript(interaction: discord.Interaction):
        await interaction.response.send_message("📄 Transcript:", file=await _generate_transcript(interaction.channel))
