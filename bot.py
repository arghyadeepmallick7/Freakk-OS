...
async def _send_to_vc_chat(voice_channel: discord.VoiceChannel, content: str) -> bool:
    ...
    try:
        ...
        log.warning(
            "VC notify: HTTP error for voice channel '%s' (%s): %s",
            getattr(voice_channel, "name", "?"),
            getattr(voice_channel, "id", "?"),
            e,
        )
...
class Freakos(commands.Bot):
    ...
    # ---------- VC notifications (per-VC, own text chat only) ----------
...
def register_all_commands(bot: Freakos):
    ...
    async def _vc_cfg(gid: int, vc_id: int) -> dict:
    ...
    @vcnotify.command(name="setup",
                      description="Enable VC notifications for a voice channel "
                                  "(uses its own text chat).")
    async def v_setup(interaction: discord.Interaction,
        ...
        await interaction.response.send_message(
            f"✅ {voice_channel.mention} → its own text chat.", ephemeral=True)
    ...
    @vcnotify.command(name="add",
                      description="Add a voice channel to watch "
                                  "(notifications go to its own text chat).")
    async def v_add(interaction: discord.Interaction, voice_channel: discord.VoiceChannel):
        ...
        await _vc_save_cfg(interaction.guild.id, voice_channel.id, cfg)
        await interaction.response.send_message(
            f"✅ Watching {voice_channel.mention} "
            f"(destination: its own text chat).", ephemeral=True)
    ...
    async def _edit_vc_msg(interaction: discord.Interaction,
        ...
        async def save(i: discord.Interaction, v: str):
            ...
            c2[key] = v
            await _vc_save_cfg(i.guild.id, vc.id, c2)
    ...
    async def v_test(interaction: discord.Interaction, voice_channel: discord.VoiceChannel):
        ...
        if ok:
            await interaction.response.send_message(
                f"✅ Sent to {voice_channel.mention}'s text chat.", ephemeral=True)
        else:
            await interaction.response.send_message(
                f"❌ Could not send to {voice_channel.mention}'s text chat. "
                f"The voice channel may not have a text chat enabled, or I "
                f"lack Send Messages permission there.", ephemeral=True)
    ...
    async def t_panel(interaction: discord.Interaction,
        ...
        panel_title = title or "Support Tickets"
        panel_description = description if description is not None else "Select a ticket type below to open a ticket."
        # Slash-command input can arrive with literal \n escapes. Convert only those
        # escapes here so real newlines remain untouched everywhere else.
        panel_description = panel_description.replace("\\r\\n", "\r\n").replace("\\n", "\n")
        embed = make_embed(title=panel_title, description=panel_description)
